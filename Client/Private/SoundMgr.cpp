#include "pch.h"
#include "SoundMgr.h"

#include <corecrt_io.h>


SoundMgr* SoundMgr::m_pInstance = nullptr;
SoundMgr::SoundMgr()
{
	m_pSystem = nullptr; 
}


SoundMgr::~SoundMgr()
{
	Release(); 
}

void SoundMgr::Initialize()
{
	// 사운드를 담당하는 대표객체를 생성하는 함수
	FMOD_System_Create(&m_pSystem, FMOD_VERSION);
	
	// 1. 시스템 포인터, 2. 사용할 가상채널 수 , 초기화 방식) 
	FMOD_System_Init(m_pSystem, 512, FMOD_INIT_NORMAL, nullptr);

	LoadSoundFile(); 
}
void SoundMgr::Release()
{
	for (auto& Mypair : m_mapSound)
	{
		delete[] Mypair.first;
		FMOD_Sound_Release(Mypair.second);
	}
	m_mapSound.clear(); 

	FMOD_System_Release(m_pSystem);
	FMOD_System_Close(m_pSystem);
}

void SoundMgr::PlaySound(const TCHAR * pSoundKey, CHANNELID eID, float fVolume)
{
	// iter = find_if(m_mapSound.begin(), m_mapSound.end(), CTag_Finder(pSoundKey));
	std::map<TCHAR*, FMOD_SOUND*>::iterator iter = find_if(
		m_mapSound.begin(), m_mapSound.end(),
		[&](auto& it)-> bool
		{
			return !lstrcmp(pSoundKey, it.first);
		});
	
	if (iter == m_mapSound.end())
		return;

	FMOD_BOOL bPlay = FALSE;

	if (FMOD_Channel_IsPlaying(m_pChannelArr[eID], &bPlay))
	{
		FMOD_System_PlaySound(m_pSystem, iter->second, nullptr, FALSE, &m_pChannelArr[eID]);
	}

	FMOD_Channel_SetMode(m_pChannelArr[eID], FMOD_DEFAULT);
	FMOD_Channel_SetVolume(m_pChannelArr[eID], fVolume);

	FMOD_System_Update(m_pSystem);
	UpdateChannelList();
}

FMOD_CHANNEL* SoundMgr::PlaySound(const TCHAR* pSoundKey, float fVolume)
{
	std::map<TCHAR*, FMOD_SOUND*>::iterator iter = find_if(
		m_mapSound.begin(), m_mapSound.end(),
		[&](auto& it)-> bool
		{
			return !lstrcmp(pSoundKey, it.first);
		});

	if (iter == m_mapSound.end())
		return nullptr;

	FMOD_CHANNEL* channel;

	FMOD_System_PlaySound(m_pSystem, iter->second, nullptr, FALSE, &channel);
	FMOD_Channel_SetMode(channel, FMOD_DEFAULT);
	FMOD_Channel_SetVolume(channel, fVolume);
	m_ChannelList.push_back(channel);

	FMOD_System_Update(m_pSystem);
	UpdateChannelList();
	return channel;
}

void SoundMgr::PlayLoopSound(const TCHAR* pSoundKey, CHANNELID eID, float fVolume)
{
	std::map<TCHAR*, FMOD_SOUND*>::iterator iter;

	// iter = find_if(m_mapSound.begin(), m_mapSound.end(), CTag_Finder(pSoundKey));
	iter = find_if(m_mapSound.begin(), m_mapSound.end(),
		[&](auto& it)->bool
		{
			return !lstrcmp(pSoundKey, it.first);
		});

	if (iter == m_mapSound.end())
		return;

	FMOD_BOOL bPlay = FALSE;

	if (FMOD_Channel_IsPlaying(m_pChannelArr[eID], &bPlay))
	{
		FMOD_System_PlaySound(m_pSystem, iter->second, nullptr, FALSE, &m_pChannelArr[eID]);
	}

	FMOD_Channel_SetMode(m_pChannelArr[eID], FMOD_LOOP_NORMAL);
	FMOD_Channel_SetVolume(m_pChannelArr[eID], fVolume);
	FMOD_System_Update(m_pSystem);
	UpdateChannelList();
}

FMOD_CHANNEL* SoundMgr::PlayLoopSound(const TCHAR* pSoundKey, float fVolume)
{
	std::map<TCHAR*, FMOD_SOUND*>::iterator iter;

	// iter = find_if(m_mapSound.begin(), m_mapSound.end(), CTag_Finder(pSoundKey));
	iter = find_if(m_mapSound.begin(), m_mapSound.end(),
		[&](auto& iter)->bool
		{
			return !lstrcmp(pSoundKey, iter.first);
		});

	if (iter == m_mapSound.end())
		return nullptr;

	FMOD_CHANNEL* channel;

	FMOD_System_PlaySound(m_pSystem, iter->second, nullptr, FALSE, &channel);
	FMOD_Channel_SetMode(channel, FMOD_LOOP_NORMAL);
	FMOD_Channel_SetVolume(channel, fVolume);
	FMOD_System_Update(m_pSystem);
	m_ChannelList.push_back(channel);
	UpdateChannelList();

	return channel;
}

void SoundMgr::PlayBGM(const TCHAR * pSoundKey, float fVolume)
{
	std::map<TCHAR*, FMOD_SOUND*>::iterator iter;

	// iter = find_if(m_mapSound.begin(), m_mapSound.end(), CTag_Finder(pSoundKey));
	iter = find_if(m_mapSound.begin(), m_mapSound.end(), [&](auto& iter)->bool
	{
		return !lstrcmp(pSoundKey, iter.first);
	});
	
	if (iter == m_mapSound.end())
		return;

	FMOD_System_PlaySound(m_pSystem, iter->second, nullptr, FALSE, &m_pChannelArr[SOUND_BGM]);
	FMOD_Channel_SetMode(m_pChannelArr[SOUND_BGM], FMOD_LOOP_NORMAL);
	FMOD_Channel_SetVolume(m_pChannelArr[SOUND_BGM], fVolume);
	FMOD_System_Update(m_pSystem);
	UpdateChannelList();
}

void SoundMgr::StopSound(CHANNELID eID)
{
	FMOD_Channel_Stop(m_pChannelArr[eID]);
}

void SoundMgr::StopSound(FMOD_CHANNEL* channel)
{
	FMOD_Channel_Stop(channel);

	for (auto it = m_ChannelList.begin(); it != m_ChannelList.end();)
	{
		if (*it == channel)
		{
			it = m_ChannelList.erase(it);
		}

		else
		{
			++it;
		}
	}
}

void SoundMgr::StopAll()
{
	for (int i = 0 ; i < MAXCHANNEL ; ++i)
		FMOD_Channel_Stop(m_pChannelArr[i]);

	for (auto channel : m_ChannelList)
	{
		FMOD_Channel_Stop(channel);
	}
	FMOD_System_Update(m_pSystem);
	UpdateChannelList();
}

void SoundMgr::SetChannelVolume(CHANNELID eID, float fVolume)
{
	FMOD_Channel_SetVolume(m_pChannelArr[eID], fVolume);

	FMOD_System_Update(m_pSystem);
}

bool SoundMgr::IsPlaying(CHANNELID eID)
{
	FMOD_BOOL bPlay = FALSE;

	FMOD_Channel_IsPlaying(m_pChannelArr[eID], &bPlay);

	if (bPlay == 1) return true;
	return false;
}

bool SoundMgr::IsPlaying(FMOD_CHANNEL* channel)
{
	FMOD_BOOL bPlay = FALSE;

	FMOD_Channel_IsPlaying(channel, &bPlay);

	if (bPlay == 1) return true;
	return false;
}

void SoundMgr::UpdateChannelList()
{
	for (auto it = m_ChannelList.begin(); it != m_ChannelList.end();)
	{
		FMOD_BOOL isPlaying;

		FMOD_Channel_IsPlaying((*it), &isPlaying);

		if (!isPlaying)
		{
			it = m_ChannelList.erase(it);
		}

		else
		{
			++it;
		}
	}
}

void SoundMgr::LoadSoundFile()
{
	// _finddata_t : <io.h>에서 제공하며 파일 정보를 저장하는 구조체
	_finddata_t fd; 

	// _findfirst : <io.h>에서 제공하며 사용자가 설정한 경로 내에서 가장 첫 번째 파일을 찾는 함수
	long long handle = _findfirst("../Client/Assets/Resource/Sound/*.*", &fd);

	if (handle == -1)
		return; 

	int iResult = 0; 

	char szCurPath[128] = "../Client/Assets/Resource/Sound/";	 // 상대 경로
	char szFullPath[128] = ""; 

	while (iResult != -1)
	{
		strcpy_s(szFullPath, szCurPath); 
		
		// "./Sound/" + "Success.wav"
		strcat_s(szFullPath, fd.name);
		// "./Sound/Success.wav"

		FMOD_SOUND* pSound = nullptr; 

		FMOD_RESULT eRes = FMOD_System_CreateSound(m_pSystem, szFullPath, FMOD_DEFAULT, nullptr, &pSound);

		if (eRes == FMOD_OK)
		{
			int iLength = strlen(fd.name) + 1; 

			TCHAR* pSoundKey = new TCHAR[iLength];
			ZeroMemory(pSoundKey, sizeof(TCHAR) * iLength);

			// 아스키 코드 문자열을 유니코드 문자열로 변환시켜주는 함수
			MultiByteToWideChar(CP_ACP, 0, fd.name, iLength, pSoundKey, iLength);

			m_mapSound.emplace(pSoundKey, pSound);
		}
		//_findnext : <io.h>에서 제공하며 다음 위치의 파일을 찾는 함수, 더이상 없다면 -1을 리턴
		iResult = _findnext(handle, &fd);
	}

	FMOD_System_Update(m_pSystem);

	_findclose(handle);
}
