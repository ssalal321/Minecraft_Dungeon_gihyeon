...
void CSkeleton_Dead::State_Update(_float fTimeDelta)
{
	...
	if (false == m_pSkeleton->Get_Can_be_Eaten() || 0 >= m_pSkeleton->Get_Eating_BossMonster()->Get_MonsterInfo()->Get_CurrentHP())
	{
	...
	else if (true == m_pSkeleton->Get_Can_be_Eaten() && !m_bCurveFinished)
	{
