...
_bool CSoggySwampMap::m_bLineRender = false;
...
CSoggySwampMap::CSoggySwampMap(const CSoggySwampMap& Prototype)
	: CGameObject ( Prototype )
{
}

HRESULT CSoggySwampMap::Initialize_Prototype()
{
...
HRESULT CSoggySwampMap::Initialize(void* pArg)
{
	/* 원형의 데이터를 복제하여 사본을 만들고. */
	/* 추가적으로 필요한 데이터를 Arg로 받아와 실 사용하기위한 객체의 정보를 생성해준다. */	
	CGameObject::GAMEOBJECT_DESC		Desc{};

	Desc.strGameObjectTag = TEXT("GameObject_SoggySwampMap");
...
void CSoggySwampMap::Priority_Update(_float fTimeDelta)
{
...
void CSoggySwampMap::Late_Update(_float fTimeDelta)
{
...
HRESULT CSoggySwampMap::Ready_Components()
{
	/* Com_Shader_Mesh */
	...
	if (nullptr == Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Model_SoggySwampMap"),
	...
	//SphereCollDesc.vCenter = _float3(2.5f, 5.5f, 25.f);
	...
	//m_pGameInstance->Add_ColliderCom(pColliderSphereCom, TEXT("SoggySwampMap_SceneChange_Collider"), TEXT("SoggySwampMap"));

	return S_OK;
}

HRESULT CSoggySwampMap::Bind_ShaderResources()
{
...
CSoggySwampMap* CSoggySwampMap::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CSoggySwampMap* pGameInstance = new CSoggySwampMap(pDevice, pContext);

	if (FAILED(pGameInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Create : CSoggySwampMap");
...
CGameObject* CSoggySwampMap::Clone(void* pArg)
{
	CSoggySwampMap* pGameInstance = new CSoggySwampMap(*this);

	if (FAILED(pGameInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Clone : CSoggySwampMap");
		Safe_Release(pGameInstance);
	}
