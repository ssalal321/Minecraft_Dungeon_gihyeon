...
CMonster_Arrow::CMonster_Arrow(const CMonster_Arrow& Prototype)
...
{
	const _wstring& arrowGameObjectTag = TEXT("GameObject_MonsterArrow_") + to_wstring(m_iArrowID++);
...
void CMonster_Arrow::Shoot(_float4 startPos, _float4 lookPos)
{
	startPos.y += 1.f;
...
{
	CMonster_Arrow* pGameInstance = new CMonster_Arrow(pDevice, pContext);

	if (FAILED(pGameInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Create : CMonster_Arrow");
...
{
	CMonster_Arrow* pGameInstance = new CMonster_Arrow(*this);

	if (FAILED(pGameInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Clone : CMonster_Arrow");
		Safe_Release(pGameInstance);
	}
