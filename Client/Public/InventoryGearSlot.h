...
class CInventoryGearSlot final : public CUIObject
{
public:
	typedef struct INVENTORY_GEARSLOT_DESC : public CUIObject::UIOBJECT_DESC
	{
		_float			fPlayTime;

		INVENTORY_GEARSLOT_DESC(const _tchar* GameObjectTag, UI_STATE uiState,
		...
		// 복사 생성자
		INVENTORY_GEARSLOT_DESC(const INVENTORY_GEARSLOT_DESC& other)
			: UIOBJECT_DESC(other), fPlayTime(other.fPlayTime) { }

		~INVENTORY_GEARSLOT_DESC() override = default;
...
private:
	INVENTORY_GEARSLOT_DESC*	m_pDesc			= { nullptr };
