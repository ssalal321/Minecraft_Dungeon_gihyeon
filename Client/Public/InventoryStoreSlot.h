...
class CInventoryStoreSlot final : public CUIObject
{
public:
	typedef struct INVENTORY_STORESLOT_DESC : public CUIObject::UIOBJECT_DESC
	{
		_float			fPlayTime;

		INVENTORY_STORESLOT_DESC(const _tchar* GameObjectTag, UI_STATE uiState,
		...
		// 복사 생성자
		INVENTORY_STORESLOT_DESC(const INVENTORY_STORESLOT_DESC& other)
			: UIOBJECT_DESC(other), fPlayTime(other.fPlayTime) { }

		~INVENTORY_STORESLOT_DESC() override = default;
...
private:
	INVENTORY_STORESLOT_DESC*	m_pDesc			= { nullptr };
